...
//                .prettyPeek()
    ...
    @DisplayName("Testing the /region endpoint")
    @Test
...
//                .get("/regions") ;
        ...
        System.out.println("First region_name = " + jsonPath.getString("items[0].region_name") );

        // Get last region_id from the response using jsonPath
        System.out.println("Last region_id = " + jsonPath.getInt("items[3].region_id") );

        // or use -1 for index to get last one
        System.out.println("Last region_id = " + jsonPath.getInt("items[-1].region_id") );

        // get the list of region name from the response and save it to List<String>
        List<String> allNames = jsonPath.getList("items.region_name");
